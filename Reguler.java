...
public class Reguler extends Pelanggan implements Interface1 {
    Film[] p;

    Reguler(int kode, String nama, int umur, String noTelp, String status) throws UmurNegatifException{
        this.kode = kode;
        this.nama = nama;
        this.umur = umur;
        this.noTelp = noTelp;
        this.status = status;
        super.kategoriPel = "Reguler";
        if (this.umur < 0) {
    ...
    public void printFilm() {
        ...
        for (int i = 0; i < pool.length; i++) {
            if (pool[i].filmDetail[5].equals("REGULER")) {
                p[y] = pool[i];
